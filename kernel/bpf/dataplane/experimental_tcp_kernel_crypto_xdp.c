...
// XDP classifier plus RX-side kernel AEAD open for TrustIX experimental_tcp.
...
#define TRUSTIX_EXP_TCP_STATS_KERNEL_CRYPTO_OPEN_ATTEMPTS 4
#define TRUSTIX_EXP_TCP_STATS_KERNEL_CRYPTO_OPEN_SUCCESSES 5
#define TRUSTIX_EXP_TCP_STATS_KERNEL_CRYPTO_OPEN_ERRORS 6
#define TRUSTIX_EXP_TCP_STATS_KERNEL_CRYPTO_REPLAY_DROPS 7
#define TRUSTIX_EXP_TCP_STATS_KERNEL_CRYPTO_NO_CONTEXT_DROPS 8
#define TRUSTIX_EXP_TCP_STATS_KERNEL_CRYPTO_HEADER_ERRORS 9
#define TRUSTIX_EXP_TCP_STATS_KERNEL_CRYPTO_DEFERRED_TO_USERSPACE 10
#define TRUSTIX_EXP_TCP_STATS_KERNEL_CRYPTO_TCP_CHECKSUM_SKIPPED 11
...
#define TRUSTIX_EXP_TCP_STATS_KERNEL_CRYPTO_DIRECT_OPEN_SUCCESSES 31
#define TRUSTIX_EXP_TCP_STATS_KERNEL_CRYPTO_DIRECT_OPEN_FALLBACKS 32
#define TRUSTIX_EXP_TCP_STATS_KERNEL_CRYPTO_FALLBACK_OPEN_SUCCESSES 33
#define TRUSTIX_EXP_TCP_STATS_KERNEL_CRYPTO_PAYLOAD_LEN_ERRORS 34
#define TRUSTIX_EXP_TCP_STATS_KERNEL_CRYPTO_SECURE_HEADER_ERRORS 35
#define TRUSTIX_EXP_TCP_STATS_KERNEL_CRYPTO_FRAME_HEADER_ERRORS 36
#define TRUSTIX_EXP_TCP_STATS_KERNEL_CRYPTO_EPOCH_SEQUENCE_MISMATCHES 37
#define TRUSTIX_EXP_TCP_STATS_KERNEL_CRYPTO_CIPHER_LEN_ERRORS 38
#define TRUSTIX_EXP_TCP_STATS_KERNEL_CRYPTO_CIPHER_LOAD_ERRORS 39
#define TRUSTIX_EXP_TCP_STATS_KERNEL_CRYPTO_CONTEXT_MISSES 40
#define TRUSTIX_EXP_TCP_STATS_KERNEL_CRYPTO_STATE_MISSES 41
#define TRUSTIX_EXP_TCP_STATS_KERNEL_CRYPTO_ZERO_PLAIN_ERRORS 42
#define TRUSTIX_EXP_TCP_STATS_KERNEL_CRYPTO_CONTEXT_UNAVAILABLE 43
#define TRUSTIX_EXP_TCP_STATS_KERNEL_CRYPTO_EPOCH_MISMATCHES 44
#define TRUSTIX_EXP_TCP_STATS_KERNEL_CRYPTO_SUITE_MISMATCHES 45
#define TRUSTIX_EXP_TCP_STATS_KERNEL_CRYPTO_DYNPTR_ERRORS 46
#define TRUSTIX_EXP_TCP_STATS_KERNEL_CRYPTO_DECRYPT_ERRORS 47
#define TRUSTIX_EXP_TCP_STATS_KERNEL_CRYPTO_REPLAY_COMMIT_ERRORS 48
#define TRUSTIX_EXP_TCP_STATS_KERNEL_CRYPTO_STORE_ERRORS 49
...
#define TRUSTIX_EXP_TCP_TCP_CSUM_HEADER_LEN (12 + TRUSTIX_EXP_TCP_TCP_HEADER_LEN + TRUSTIX_EXP_TCP_HEADER_LEN)
...
#define TRUSTIX_EXP_TCP_CONFIG_KERNEL_UDP_XDP_OPEN 4
#define TRUSTIX_EXP_TCP_CONFIG_KERNEL_UDP_XDP_PASS_OPENED 8
...
#if defined(TRUSTIX_EXP_TCP_DIRECT_OPEN) && !defined(TRUSTIX_EXP_TCP_DIRECT_OPEN_KEEP_FALLBACK)
#define TRUSTIX_EXP_TCP_DIRECT_OPEN_ONLY 1
#endif

#if defined(TRUSTIX_EXP_TCP_DIRECT_OPEN_ONLY) && !defined(TRUSTIX_EXP_TCP_DIRECT_OPEN_KEEP_XDP_RX_DIRECT)
#define TRUSTIX_EXP_TCP_NO_XDP_RX_DIRECT 1
#endif
...
struct trustix_exp_tcp_kernel_crypto_scratch {
...
struct trustix_exp_tcp_tcp_csum_header {
    __u8 pseudo[12];
...
struct {
    ...
    __uint(max_entries, 1);
    __type(key, __u32);
    __type(value, struct trustix_exp_tcp_kernel_crypto_scratch);
} ix_exp_tcp_kernel_crypto_scratch SEC(".maps");
...
#ifndef TRUSTIX_EXP_TCP_DIRECT_OPEN_ONLY
...
static __always_inline __u32 trustix_exp_tcp_load_config(void)
{
    __u32 key = 0;
    __u32 *config = bpf_map_lookup_elem(&ix_exp_tcp_config, &key);

    return config ? *config : 0;
}

static __always_inline int trustix_exp_tcp_config_hot_path_stats(__u32 config)
{
    return config & TRUSTIX_EXP_TCP_CONFIG_HOT_PATH_STATS;
}

static __always_inline void trustix_exp_tcp_count_hot_config(__u32 config, __u32 key)
{
    if (trustix_exp_tcp_config_hot_path_stats(config))
        trustix_exp_tcp_count(key);
}

static __always_inline int trustix_exp_tcp_hot_path_stats(void)
{
    __u32 key = 0;
    __u32 *config = bpf_map_lookup_elem(&ix_exp_tcp_config, &key);

    return config && (*config & TRUSTIX_EXP_TCP_CONFIG_HOT_PATH_STATS);
}

static __always_inline void trustix_exp_tcp_count_hot(__u32 key)
{
    if (trustix_exp_tcp_hot_path_stats())
        trustix_exp_tcp_count(key);
}
    ...
    __u32 source_mac0, __u16 source_mac1, __u32 ifindex, __u32 config)
{
    ...
    *(__u16 *)(data + 10) = source_mac1;
    data[12] = 0x08;
    data[13] = 0x00;
    trustix_exp_tcp_count_hot_config(config, TRUSTIX_EXP_TCP_STATS_KERNEL_UDP_XDP_RX_DIRECT_REDIRECTS);
    if ((config & TRUSTIX_EXP_TCP_CONFIG_KERNEL_UDP_XDP_RX_DIRECT_IFINDEX) && ifindex) {
        trustix_exp_tcp_count_hot_config(config, TRUSTIX_EXP_TCP_STATS_KERNEL_UDP_XDP_RX_DIRECT_IFINDEX_REDIRECTS);
        return bpf_redirect(ifindex, 0);
    }
    trustix_exp_tcp_count_hot_config(config, TRUSTIX_EXP_TCP_STATS_KERNEL_UDP_XDP_RX_DIRECT_DEVMAP_REDIRECTS);
    ...
    struct xdp_md *ctx, __u32 inner_offset, __u32 config)
{
    ...
    trustix_exp_tcp_count_hot_config(config, TRUSTIX_EXP_TCP_STATS_KERNEL_UDP_XDP_RX_DIRECT_CANDIDATES);
    ...
    if (inner_len < 20 || inner_len > available)
        goto direct_len_error;
    if (!(config & TRUSTIX_EXP_TCP_CONFIG_KERNEL_UDP_XDP_RX_DIRECT_TRUST_INNER_CHECKSUM) &&
    ...
    if (config & TRUSTIX_EXP_TCP_CONFIG_KERNEL_UDP_XDP_RX_DIRECT_FIXED_L2) {
        __u32 config_key = 0;
        ...
        trustix_exp_tcp_count_hot_config(config, TRUSTIX_EXP_TCP_STATS_KERNEL_UDP_XDP_RX_DIRECT_BROADCASTS);
    ...
    trustix_exp_tcp_count_hot_config(config, TRUSTIX_EXP_TCP_STATS_KERNEL_UDP_XDP_RX_DIRECT_NEIGH_HITS);
    return trustix_kernel_udp_xdp_redirect_lan(
        ctx, inner_offset, inner_len, available,
...
static __always_inline __u32 trustix_exp_tcp_redirect_queue_config(struct xdp_md *ctx, __u32 config)
{
    ...
    trustix_exp_tcp_count_hot_config(config, TRUSTIX_EXP_TCP_STATS_QUEUE_FALLBACK);
    return 0;
}
...
static __noinline int trustix_fix_tcp_checksum(__u8 *ip, __u8 *tcp, __u8 *frame,
                                               __u8 *data_end,
                                               struct trustix_exp_tcp_kernel_crypto_scratch *scratch)
{
    struct trustix_exp_tcp_tcp_csum_header header = {};
    __u32 plain_len;
    __u32 tcp_len;
    __u32 plain_padded;
    __u32 sum32;
    ...
    if (plain_len > TRUSTIX_KERNEL_CRYPTO_FRAME_MAX - TRUSTIX_KERNEL_CRYPTO_FRAME_TAG_LEN)
        return -22;
    tcp_len = TRUSTIX_EXP_TCP_TCP_HEADER_LEN + TRUSTIX_EXP_TCP_HEADER_LEN + plain_len;
...
#pragma clang loop unroll(full)
    ...
    if (plain_padded > TRUSTIX_EXP_TCP_CSUM_CHUNK) {
        chunk = plain_padded - TRUSTIX_EXP_TCP_CSUM_CHUNK;
        if (chunk > TRUSTIX_EXP_TCP_CSUM_CHUNK)
            chunk = TRUSTIX_EXP_TCP_CSUM_CHUNK;
        if (trustix_add_csum_chunk(scratch->plain + TRUSTIX_EXP_TCP_CSUM_CHUNK, chunk, &sum32))
            return -22;
    }
    if (plain_padded > TRUSTIX_EXP_TCP_CSUM_CHUNK * 2) {
        chunk = plain_padded - TRUSTIX_EXP_TCP_CSUM_CHUNK * 2;
        if (chunk > TRUSTIX_EXP_TCP_CSUM_CHUNK)
            chunk = TRUSTIX_EXP_TCP_CSUM_CHUNK;
        if (trustix_add_csum_chunk(scratch->plain + TRUSTIX_EXP_TCP_CSUM_CHUNK * 2, chunk, &sum32))
            return -22;
    }
    if (plain_padded > TRUSTIX_EXP_TCP_CSUM_CHUNK * 3) {
        chunk = plain_padded - TRUSTIX_EXP_TCP_CSUM_CHUNK * 3;
        if (chunk > TRUSTIX_EXP_TCP_CSUM_CHUNK)
            chunk = TRUSTIX_EXP_TCP_CSUM_CHUNK;
        if (trustix_add_csum_chunk(scratch->plain + TRUSTIX_EXP_TCP_CSUM_CHUNK * 3, chunk, &sum32))
            return -22;
    }
                                         ...
                                         struct trustix_exp_tcp_kernel_crypto_scratch *scratch)
{
    ...
    if (payload_len < TRUSTIX_EXP_TCP_OVERHEAD ||
        ...
        trustix_exp_tcp_count(TRUSTIX_EXP_TCP_STATS_KERNEL_CRYPTO_PAYLOAD_LEN_ERRORS);
        return -22;
    }
    if (payload + TRUSTIX_KERNEL_CRYPTO_SECURE_HEADER_LEN > data_end) {
        trustix_exp_tcp_count(TRUSTIX_EXP_TCP_STATS_KERNEL_CRYPTO_SECURE_HEADER_ERRORS);
        return -14;
    }
    ...
    if (!frame || frame + TRUSTIX_KERNEL_UDP_HEADER_LEN > data_end) {
        trustix_exp_tcp_count(TRUSTIX_EXP_TCP_STATS_KERNEL_CRYPTO_FRAME_HEADER_ERRORS);
        return -14;
    }
    ...
    if (trustix_read_be64(payload + 8) != epoch ||
        ...
        trustix_exp_tcp_count(TRUSTIX_EXP_TCP_STATS_KERNEL_CRYPTO_EPOCH_SEQUENCE_MISMATCHES);
        return -74;
    }
    ...
    if (bounded_cipher_len != raw_cipher_len ||
        ...
        trustix_exp_tcp_count(TRUSTIX_EXP_TCP_STATS_KERNEL_CRYPTO_CIPHER_LEN_ERRORS);
        return -22;
    }
    cipher_len = bounded_cipher_len;
    plain_len = cipher_len - TRUSTIX_KERNEL_CRYPTO_FRAME_TAG_LEN;
    if (!scratch) {
        trustix_exp_tcp_count(TRUSTIX_EXP_TCP_STATS_KERNEL_CRYPTO_CONTEXT_MISSES);
        return -2;
    }
    scratch->plain_len = plain_len;

    if (!ctx || payload < data) {
        trustix_exp_tcp_count(TRUSTIX_EXP_TCP_STATS_KERNEL_CRYPTO_CIPHER_LOAD_ERRORS);
        return -14;
    }
...
#endif
    if (plain_len == 0) {
        trustix_exp_tcp_count(TRUSTIX_EXP_TCP_STATS_KERNEL_CRYPTO_ZERO_PLAIN_ERRORS);
        return -22;
    }
    ...
    if (direct_slot && direct_slot->enabled) {
        if (direct_slot->epoch != epoch) {
            trustix_exp_tcp_count(TRUSTIX_EXP_TCP_STATS_KERNEL_CRYPTO_EPOCH_MISMATCHES);
            return -74;
        }
        ...
        if (!err) {
            ...
            trustix_exp_tcp_count_hot(TRUSTIX_EXP_TCP_STATS_KERNEL_CRYPTO_DIRECT_OPEN_SUCCESSES);
            goto direct_opened;
        }
        trustix_exp_tcp_count_hot(TRUSTIX_EXP_TCP_STATS_KERNEL_CRYPTO_DIRECT_OPEN_FALLBACKS);
    }
#ifdef TRUSTIX_EXP_TCP_DIRECT_OPEN_ONLY
    trustix_exp_tcp_count(TRUSTIX_EXP_TCP_STATS_KERNEL_CRYPTO_CONTEXT_UNAVAILABLE);
    return -2;
#endif
#endif
...
#ifndef TRUSTIX_EXP_TCP_DIRECT_OPEN_ONLY
    if (!state) {
        trustix_exp_tcp_count(TRUSTIX_EXP_TCP_STATS_KERNEL_CRYPTO_STATE_MISSES);
        return -2;
    }
    ...
    if (bpf_dynptr_from_mem(scratch->cipher, cipher_len, 0, &cipher) ||
        ...
        trustix_exp_tcp_count(TRUSTIX_EXP_TCP_STATS_KERNEL_CRYPTO_DYNPTR_ERRORS);
        err = -14;
        goto out_unlock;
    }
...
direct_opened:
    ...
    if (payload + plain_len > data_end) {
        trustix_exp_tcp_count(TRUSTIX_EXP_TCP_STATS_KERNEL_CRYPTO_STORE_ERRORS);
        return -14;
    }
...
int trustix_exp_tcp(struct xdp_md *ctx)
{
    ...
    if (encrypted_flag && (frame[5] & TRUSTIX_EXP_TCP_FLAG_CRYPTO_FRAGMENT))
        goto redirect;
    if (encrypted_flag) {
        if (data + 14 + 20 + 20 + TRUSTIX_EXP_TCP_HEADER_LEN + TRUSTIX_EXP_TCP_OVERHEAD > data_end)
            goto encrypted_header_error;
    } else if (frame[5] & TRUSTIX_EXP_TCP_FLAG_KERNEL_OPENED) {
        goto parse_error;
    }
    ...
    if (payload_len > TRUSTIX_KERNEL_CRYPTO_SECURE_HEADER_LEN + TRUSTIX_KERNEL_CRYPTO_FRAME_MAX)
        goto parse_error;
    if (frame + TRUSTIX_EXP_TCP_HEADER_LEN + payload_len > data_end)
        goto parse_error;
    ...
    if (encrypted_flag) {
        ...
        trustix_exp_tcp_count_hot_config(config, TRUSTIX_EXP_TCP_STATS_KERNEL_CRYPTO_OPEN_ATTEMPTS);
        plain_len = trustix_open_frame(ctx, payload, payload_len, frame, scratch);
        if (plain_len < 0) {
            if (plain_len == -114) {
                trustix_exp_tcp_count(TRUSTIX_EXP_TCP_STATS_KERNEL_CRYPTO_REPLAY_DROPS);
                return XDP_DROP;
            } else if (plain_len == -2) {
                trustix_exp_tcp_count(TRUSTIX_EXP_TCP_STATS_KERNEL_CRYPTO_DEFERRED_TO_USERSPACE);
                goto redirect;
            } else if (plain_len == -22 || plain_len == -74)
                trustix_exp_tcp_count(TRUSTIX_EXP_TCP_STATS_KERNEL_CRYPTO_HEADER_ERRORS);
            trustix_exp_tcp_count(TRUSTIX_EXP_TCP_STATS_KERNEL_CRYPTO_OPEN_ERRORS);
            return XDP_DROP;
        }
        old_total_len = ((__u32)ip[2] << 8) | ip[3];
        if (old_total_len < 20 + 20 + TRUSTIX_EXP_TCP_HEADER_LEN + payload_len)
            goto parse_error;
        if (ip + old_total_len > data_end)
            goto parse_error;
        opened_inner_ipv4 = (frame[5] & TRUSTIX_EXP_TCP_FLAG_INNER_IPV4) &&
                            trustix_exp_tcp_unfragmented(frame) &&
        ...
        pass_opened_to_tc = opened_inner_ipv4 &&
                            ...
                            (config & TRUSTIX_EXP_TCP_CONFIG_KERNEL_UDP_XDP_PASS_OPENED);
        new_total_len = old_total_len - (payload_len - (__u32)plain_len);
        frame[5] = (frame[5] & ~TRUSTIX_EXP_TCP_FLAG_ENCRYPTED) | TRUSTIX_EXP_TCP_FLAG_KERNEL_OPENED;
        trustix_write_be32(frame + 32, (__u32)plain_len);
        trustix_write_be16(ip + 2, (__u16)new_total_len);
        trustix_fix_ipv4_checksum(ip);
        tcp[16] = 0;
        tcp[17] = 0;
        if (config & TRUSTIX_EXP_TCP_CONFIG_SKIP_TCP_CHECKSUM) {
            trustix_exp_tcp_count_hot_config(config, TRUSTIX_EXP_TCP_STATS_KERNEL_CRYPTO_TCP_CHECKSUM_SKIPPED);
        ...
        if (bpf_xdp_adjust_tail(ctx, -((int)payload_len - plain_len)))
            goto parse_error;
        trustix_exp_tcp_count_hot_config(config, TRUSTIX_EXP_TCP_STATS_KERNEL_CRYPTO_OPEN_SUCCESSES);
#ifndef TRUSTIX_EXP_TCP_NO_XDP_RX_DIRECT
    ...
    if ((frame[5] == TRUSTIX_EXP_TCP_FLAG_INNER_IPV4 ||
         frame[5] == (TRUSTIX_EXP_TCP_FLAG_KERNEL_OPENED | TRUSTIX_EXP_TCP_FLAG_INNER_IPV4)) &&
        frame[36] == 0 && frame[37] == 0 && frame[38] == 0 &&
        frame[39] == 0) {
        trustix_exp_tcp_count_hot_config(config, TRUSTIX_EXP_TCP_STATS_KERNEL_UDP_PLAINTEXT_CANDIDATES);
        payload_len = trustix_read_be32(frame + 32);
        frame_end_offset = 0;
        if (payload_len <= 0xffff - 94)
            frame_end_offset = 94 + payload_len;
        if (frame_end_offset == packet_len &&
            frame + TRUSTIX_EXP_TCP_HEADER_LEN + 20 <= data_end &&
            frame[TRUSTIX_EXP_TCP_HEADER_LEN] == 0x45) {
#ifndef TRUSTIX_EXP_TCP_NO_XDP_RX_DIRECT
            if (config & TRUSTIX_EXP_TCP_CONFIG_KERNEL_UDP_XDP_RX_DIRECT) {
                direct_action = trustix_kernel_udp_xdp_rx_direct_inner(ctx, 94, config);
                if (direct_action >= 0)
                    return direct_action;
                goto redirect;
...
#endif
            if (config & TRUSTIX_EXP_TCP_CONFIG_KERNEL_UDP_TC_RX_DIRECT) {
                trustix_exp_tcp_count_hot_config(config, TRUSTIX_EXP_TCP_STATS_KERNEL_UDP_TC_RX_DIRECT_PASS);
                goto pass;
            }
        }
        if (frame_end_offset != 0 &&
            ...
            frame[TRUSTIX_EXP_TCP_HEADER_LEN] == 0x45 &&
            trustix_exp_tcp_unfragmented(frame) &&
            (config & TRUSTIX_EXP_TCP_CONFIG_KERNEL_UDP_TC_RX_DIRECT)) {
            trustix_exp_tcp_count_hot_config(config, TRUSTIX_EXP_TCP_STATS_KERNEL_UDP_TC_RX_DIRECT_PASS);
            goto pass;
        }
        trustix_exp_tcp_count_hot_config(config, TRUSTIX_EXP_TCP_STATS_KERNEL_UDP_INNER_IPV4_MISSES);
    }

redirect:
    if (config & TRUSTIX_EXP_TCP_CONFIG_XDP_FALLBACK_PASS)
        goto pass;
    trustix_exp_tcp_count_hot_config(config, TRUSTIX_EXP_TCP_STATS_REDIRECTED);
    return bpf_redirect_map(&ix_exp_tcp_xsk, trustix_exp_tcp_redirect_queue_config(ctx, config), XDP_DROP);
...
udp:
    ...
    config = trustix_exp_tcp_load_config();
    if (!trustix_exp_tcp_port_allowed(udp))
        goto drop;
    encrypted_flag = frame[5] & TRUSTIX_EXP_TCP_FLAG_ENCRYPTED;
    if (encrypted_flag &&
        (frame[5] & TRUSTIX_EXP_TCP_FLAG_INNER_IPV4) &&
    ...
    if ((frame[5] == TRUSTIX_EXP_TCP_FLAG_INNER_IPV4 ||
        ...
        if (frame + TRUSTIX_KERNEL_UDP_HEADER_LEN + 34 <= data_end &&
            ...
            trustix_exp_tcp_count_hot_config(config, TRUSTIX_EXP_TCP_STATS_KERNEL_UDP_INNER_IPV4_AT88);
#ifndef TRUSTIX_EXP_TCP_NO_XDP_RX_DIRECT
            if (config & TRUSTIX_EXP_TCP_CONFIG_KERNEL_UDP_XDP_RX_DIRECT) {
...
pass:
    if (config ? trustix_exp_tcp_config_hot_path_stats(config) : trustix_exp_tcp_hot_path_stats())
