...
### Zaozhuang PVE c412a27 TIX-TCP RX page-frag cache production refresh

Validation used disposable VM200 through VM203 on isolated `vmbr3`; VM100 and
all 1xx guests were untouched. The candidate was built from
`c412a27d7a3e354c9a4c83edb5123a38257ec210` with Go 1.25.12 at
`2026-07-30T08:22:18Z`. Every production-gate node ran the same
`trustix-linux-amd64` binary, SHA256
`60fb48d08ae333d2ce262a4c9e6619861b095eb8b43f3f61b4846647955dfc85`.
The Debian 6.12.96 module SHA256 was
`b3b977b399e94a1bf788c2af5df3dfdf5cbc7247fe9e43d0daa0bdc4fff0e23a`;
the OpenWrt 6.6.141 module SHA256 was
`5160f1c41ae64a472384f186e8b20b70c5312590b15b5e0a5d598d2d35d97fe6`.

The candidate includes two RX-side changes after the outer-GSO TX page-pool
work. `bcc9e3f` parses nonlinear TIX-TCP outer skbs by offset and copies only
the accepted inner payload instead of first linearizing the entire outer skb.
`c412a27` serves the nonlinear coalesced inner GSO payload pages from per-CPU
page-frag caches. Allocation failures retain the bounded `alloc_page()`
fallback, and module teardown drains every possible CPU cache after RX workers
and hooks are stopped.
...
| Pair and direction | Received | Sent | Intervals | Receiver duration |
| --- | ---: | ---: | ---: | ---: |
| Debian VM200 to Debian VM201 | 14.805806 Gbps | 14.805857 Gbps | 3600 | 3600.023 seconds |
| Debian VM201 to Debian VM200 | 13.555675 Gbps | 13.555755 Gbps | 3600 | 3600.035 seconds |
| OpenWrt VM202 to Debian VM203 | 5.080420 Gbps | 5.080566 Gbps | 3600 | 3600.032 seconds |
| Debian VM203 to OpenWrt VM202 | 5.287641 Gbps | 5.287756 Gbps | 3600 | 3600.076 seconds |

Both strict verifiers returned `status=pass` and `errors=[]` against the 4 Gbps
gate. All four boot IDs stayed stable, pstore was empty, kernel logs had no
rejected finding, `tix-lan` retained `tx_queue_len=1000`, and module unload was
clean. Across the four nodes, RX page-frag caches served all 4,560,055,905
attempts with zero fallback or error. TX page pools served 519,395,638 of
519,400,046 attempts; 4,408 used the existing bounded fallback and no pool
error was recorded.

A separate adjacent Debian A/B used two 120-second samples per mode. Disabling
the RX page-frag cache averaged 14.484899 Gbps. Enabling it averaged
18.239258 Gbps, a 25.9% increase, with a 19.472638 Gbps peak. This short A/B is
an engineering comparison, not a deployment-wide throughput guarantee.

OpenWrt virtio steering was tested separately with the same candidate. The
stock image placed software RPS for every virtio RX queue on one CPU
(`rps_cpus=04`) and measured 5.764637/4.873256 Gbps. Setting
`network.globals.packet_steering=0`, which left the multiqueue virtio masks at
`00`, measured 9.152166/13.180706 Gbps. Setting `packet_steering=2` and masks
`ff` measured 5.886434/4.962107 Gbps. These are 120-second A/B diagnostics.
For the tested OpenWrt x86_64 virtio multiqueue topology, disable software RPS
and let the hardware/virtio queues distribute work. Do not apply this result
unchanged to single-queue or physical NICs; inspect queue IRQ and RPS placement
on the target first.

The production evidence is pinned to gate SHA256
`985c4cb3bbc127409327a5397c5643f7d055c6c7b1ddbbb303ff0ae9d653b2aa`,
verifier SHA256
`8b67f33404150fea43019d060daab1b11d1dba1b910cbea4af509d4c9abffa9c`,
runner SHA256
`086890c1b9ea5f35cba7ecb3fd2d2f8f655b8a5f33c7c8f03627262708f2bbab`,
...
### Zaozhuang PVE 3528328 UDP userspace production gate

PVE host `120.220.44.72:8006` ran the current-head Debian-to-Debian UDP
userspace production gate on the existing disposable VM IDs 203 and 204. VM100
and all 1xx guests were not modified. Evidence artifacts are preserved on the
PVE host at:

`/root/trustix-pve-work/results/current-3528328-userspace-udp-production-20260709-0740`

Both guests ran Debian 13 kernel `6.12.94+deb13-cloud-amd64` with 4 vCPU and
virtio underlay interfaces on `eth1`. The TrustIX binary was
`trustix-linux-amd64`, commit `3528328a8935`, build time
`2026-07-08T23:29:04Z`, Go `go1.25.0`, and binary SHA256
`f18e8f479e5e25aaa4e5734042e08fb9ae9b84150075d9d3e90968ca1d5dec2b`.

The selected production gate passed both UDP userspace defaults:

| Case | Minimum received | Minimum sent | Minimum interval | Gate |
| --- | ---: | ---: | ---: | ---: |
| UDP secure userspace | 1.750550 Gbps | 1.750610 Gbps | 1.358833 Gbps | 1.5 Gbps, 3600s |
| UDP plaintext userspace | 1.814131 Gbps | 1.814188 Gbps | 1.183304 Gbps | 1.5 Gbps, 3600s |

Both cases ran `iperf_mode=forward` with `iperf_directions=both`; each
direction reached at least 3600 seconds with 3600+ interval samples. The gate
used `trustix-cross-host-production-gate-manifest-v1`, production gate SHA256
`1371160cca3cceb50617f1cae8704b1755b858bcf08ca530f32b7d46245b19d3`,
...
The initial wrapper exited nonzero only after both selected gates had passed,
because `pve-current-userspace-refresh.sh` built its default note template with
a Bash `${VAR:-...}` expansion containing `{transport}` placeholders. The
completed selected-gate artifacts were reused after fixing that template
expansion to generate the promoted `evidence.tsv`; the 3600s traffic was not
rerun.
...
### Zaozhuang PVE 8c2eebc Debian secure kernel production gates

PVE host `120.220.44.72:8006` ran fresh Debian-to-Debian secure kernel
production gates on disposable VM IDs 200 and 201 only. Both guests ran Debian
13 kernel `6.12.94+deb13-cloud-amd64`, with VM200 underlay `10.203.3.200/24`
on `eth1` and VM201 underlay `10.203.3.201/24` on `eth1`. VM100 and all 1xx
guests were not modified.
...
`0a171df97959d753eeebcb6bea17199d5a1bda69bafd2720b49259068768aee9`.
Evidence artifacts are preserved on the PVE host at
`/root/trustix-pve-work/results/current-8c2eebc-secure-kernel-production-20260705-071454`.
...
empty, the soak-window kernel journal had no entries, and the production
verifier accepted the loaded `trustix_crypto` plus `trustix_datapath_helpers`
state with route-TCP GSO/XMIT and kernel crypto counters active.
...
PVE host `120.220.44.72:8006` ran fresh Debian-to-Debian full plaintext kernel
module production gates on disposable VM IDs 200 and 201 only. VM200
`trustix-dd-aee1046-a` used underlay `10.203.3.200/24` on `eth1`; VM201
`trustix-dd-aee1046-b` used underlay `10.203.3.201/24` on `eth1`. Both ran
Debian 13 kernel `6.12.94+deb13-cloud-amd64`. VM100 and all 1xx guests were
not modified.

The TrustIX binary was `trustix-linux-amd64`, commit
`8c2eebccbcf031f0133c8dbf192d826526c5187c`, build time
`2026-07-04T11:27:03Z`, Go `go1.25.0`, binary SHA256
`e836a5e75515b81ee4874ac7e456624e8375fe4431d8a8f3b2f31e37982d2ee2`, and
embedded asset tree SHA256
`b6276fc382d9d34dffa4a957295edc27387b60a539c0195439446e385a1b732f`.
The PVE host kernel was `6.17.9-1-pve`, so host-built embedded `.ko` assets
were not promoted to the guests. Each VM rebuilt and loaded matching
`trustix_crypto.ko`, `trustix_datapath.ko`, and
`trustix_datapath_helpers.ko` under `/etc/trustix/modules` with vermagic
`6.12.94+deb13-cloud-amd64 SMP preempt mod_unload modversions`.
...
`0a171df97959d753eeebcb6bea17199d5a1bda69bafd2720b49259068768aee9`.
Evidence artifacts are preserved on the PVE host at
`/root/trustix-pve-work/results/current-8c2eebc-dd-kmod-3600s-20260704-194220`
and
`/root/trustix-pve-work/results/current-8c2eebc-expfullkmod16-3600s-20260704-235239`.
...
Both production runs kept stable boot IDs
(`46cf03ff-2df0-4f4a-926f-7112df2a1935` and
`1e81a222-1a70-45fa-a066-dbe53f3f07f6`), pstore artifacts were clean, the
production verifier reported no kernel log crash findings, `tix-lan`
`tx_queue_len` stayed at 1000, and covered module/datapath error counters were
zero.

## 2026-07-04

<a id="2026-07-04-zaozhuang-pve-add2971-route-gso-txq-backoff-production"></a>

### Zaozhuang PVE add2971 route-GSO stopped-TXQ production gate

PVE host `120.220.44.72:8006` ran a fresh Debian-to-Debian route-GSO selected
production gate on disposable VM IDs 200 and 201 only. VM200
`trustix-dd-aee1046-a` used underlay `10.203.3.200/24` on `eth1`; VM201
`trustix-dd-aee1046-b` used underlay `10.203.3.201/24` on `eth1`. Both ran
Debian 13 kernel `6.12.94+deb13-cloud-amd64`. VM100 and all 1xx guests were
not modified.
...
The TrustIX binary was `trustix-linux-amd64`, commit
`add2971946b4948fbdd49d973aa94581b2e87a50`, build time
`2026-07-04T08:21:58Z`, Go `go1.25.0`, and binary SHA256
`97ff30aa0aff7905a49249d02fb9457817324f9d4d56cb36a5d0353ad971c758`.
The production gate script SHA256 was
`e6e2c7c69807adaa8bd171b59225ce15b307c668c280b12b027baab19f12f029`;
the verifier SHA256 was
`0a171df97959d753eeebcb6bea17199d5a1bda69bafd2720b49259068768aee9`.
...
| Gate family | Minimum received | Duration | Notes |
| --- | ---: | ---: | --- |
| `route_gso` | 7.515116 Gbps | 3600s | `trustix_datapath_helpers`; stopped-TXQ backoff retries=1 sleep=50us; route-GSO async helper error counters all 0 |

Per-direction throughput was 9.519358/9.519236 Gbps A-to-B and
7.515238/7.515116 Gbps B-to-A. Boot IDs stayed stable
(`46cf03ff-2df0-4f4a-926f-7112df2a1935` and
`1e81a222-1a70-45fa-a066-dbe53f3f07f6`), pstore artifacts were clean, and the
production verifier reported no kernel log crash findings. Final route-GSO
helper counters included `route_tcp_gso_async_xmit_errors=0`,
`route_tcp_gso_async_queue_full=0`, `route_tcp_gso_async_stream_errors=0`,
and `route_tcp_gso_async_txq_stopped_drops=0` on both nodes. The only nonzero
A-side `tixt_rx_stream_coalesce_xmit_errors` matched
`tixt_rx_backlog_worker_disabled` during teardown, with
`tixt_rx_backlog_worker_queue_full=0`, so it was not a running data-path queue
overflow or route-GSO xmit failure.

## 2026-07-03

<a id="2026-07-03-zaozhuang-pve-netdevfix-openwrt24107-debian13-full-kernel-production"></a>

### Zaozhuang PVE netdevfix OpenWrt-Debian full-kernel production gate

PVE host `120.220.44.72:8006` ran fresh OpenWrt-Debian full-kernel gates on
disposable VM IDs 200 and 201 only. VM200 `trustix-owdeb-openwrt` ran
OpenWrt 24.10.7 kernel `6.6.141` at `10.203.3.200`; VM201
`trustix-owdeb-debian` ran Debian 13 kernel
`6.12.90+deb13.1-cloud-amd64` at `10.203.3.201`. Both used isolated `vmbr3`;
VM100 and all 1xx guests were not modified.

The TrustIX binary was `trustix-linux-amd64-netdevfix`, commit
`9235159503ed1746a41af9a86cbe9baebd67ed8f`, build time
`2026-07-03T09:41:34Z`, Go `go1.25.0`, and binary SHA256
`952874623231c3bd9e846f6a9810b892517e180a91272418ba470570a70e8892`.
The production gate script SHA256 was
...
Debian node recorded `trustix_datapath.rx_worker_single_coalesce=Y`. Both
production gate summaries passed with stable boot IDs, clean pstore artifacts,
no kernel log crash findings, no `unregister_netdevice` wait logs, no
`tx_queue_len` findings, and loaded full-kmod fast-path state during the runs.
...
kernel `6.12.90+deb13.1-cloud-amd64` with virtio network devices.

The release used TrustIX version `trustix-linux-amd64`, commit
`31b35f150bd9`, Go `1.25.0`, build time `2026-06-30T07:05:11Z`, assets SHA256
`4e0476a6fb315983cfb57f4dcb2c221e18d720aee4c90fc8c6c783f6d434681e`,
and binary SHA256
`e5585726a28daa6a08b506615a23b718b0cbb311517073bf20a114b8cf172bc7`.
The guests loaded `trustix_datapath.ko` SHA256
`2b065be698dd93238d7154b6210931d785a596adac835ce2e4542404df69c763`
with vermagic `6.12.90+deb13.1-cloud-amd64 SMP preempt mod_unload
modversions`.
...
`2026-06-30T09:26:59Z`, elapsed `7204` seconds, and requested `3600s` per
direction. Minimum received throughput was `7.558800 Gbps`; A-to-B received
`7.558800 Gbps` and B-to-A received `7.906989 Gbps`.

Both guests kept stable boot IDs
`93c53068-f3da-481d-b16a-3102653bd218` and
`a09899ef-232d-4a9f-868c-ecd43abcea9d`. The verifier passed with `errors=[]`,
...
### Zaozhuang PVE 928c5d0 full-kmod 3600s production gates

PVE host `120.220.44.72:8006` was used with disposable VM IDs 200+ only:
VM200 `trustix-current-a` and VM201 `trustix-current-b` ran Debian 13 kernel
`6.12.90+deb13.1-cloud-amd64`; VM202 `trustix-openwrt-202` ran OpenWrt
24.10.7 kernel `6.6.141`; VM203 `trustix-owdeb-debian` ran Debian 13 kernel
`6.12.90+deb13.1-cloud-amd64`. All test guests used virtio network devices on
isolated `vmbr3`. VM100 and all 1xx guests were not modified. The
OpenWrt-Debian production gate was started only after the Debian-Debian gate
finished, so the two production results did not share iperf load.

The release used TrustIX version `trustix-linux-amd64`, commit
`928c5d0939cc`, Go `1.25.0`, build time `2026-06-27T15:41:53Z`, binary SHA256
`a4b8662bb4619a554d595bdc7f03674723b0c8d61de29e6c819686b48d49da26`, and
embedded assets SHA256
`4e0476a6fb315983cfb57f4dcb2c221e18d720aee4c90fc8c6c783f6d434681e`.
The Debian `trustix_datapath.ko` SHA256 was
`3fabdbf7cb619bf7207ddeed1919ba5077c5fe997e41b84decbad56e74f04a47`; the
OpenWrt `trustix_datapath.ko` SHA256 was
`005fee841ca6cb82b030bd31abac799f9e9dbd7ce7d2b5ceda340612c0c91fce`.

Both selected production gates emitted
...
| Matrix | Direction | Gate | Received | Sent | Evidence seconds | Retransmits |
| --- | --- | ---: | ---: | ---: | ---: | ---: |
| Debian-Debian `full_kmod` | A to B | 3 Gbps | 3.473494 Gbps | 3.473526 Gbps | 3600.010906 | 296719 |
| Debian-Debian `full_kmod` | B to A | 3 Gbps | 3.560127 Gbps | 3.560160 Gbps | 3600.006410 | 298714 |
| OpenWrt-Debian `owdeb_full_kmod` | OpenWrt to Debian | 3 Gbps | 3.312690 Gbps | 3.312763 Gbps | 3600.026328 | 291810 |
| OpenWrt-Debian `owdeb_full_kmod` | Debian to OpenWrt | 3 Gbps | 5.060117 Gbps | 5.060189 Gbps | 3600.035104 | 54517 |

Both gates ran `forward + both` with `iperf_parallel=8`; each elapsed 7204
seconds. The Debian-Debian gate ran from `2026-06-27T15:49:33Z` to
`2026-06-27T17:49:37Z`. The OpenWrt-Debian gate ran from
`2026-06-27T17:50:52Z` to `2026-06-27T19:50:56Z`. Both gates passed with
`errors=[]`, `log_findings=[]`, `kernel_log_rejected_artifacts=[]`, and
`pstore_rejected_artifacts=[]`.

Boot IDs stayed stable before and after the runs. Debian-Debian used VM200
boot ID `2dd757ce-0433-4cdd-a67a-253e0b89afb8` and VM201 boot ID
`68bbde25-ab5c-483f-b358-32c974956f2f`; OpenWrt-Debian used VM202 boot ID
`4a7a3b0a-dce4-4c24-a512-e000d089c310` and VM203 boot ID
`29ed2d3f-cf80-49d8-9c74-4aa997e178ec`.

All peers loaded `trustix_datapath`, kept `tix-lan` `tx_queue_len=1000`, and
reported `enable_features=128`, `safe_features=128`, `unsafe_features=0`,
`selftest_failures=0`, `rx_worker_inject=Y`, `tx_plaintext=Y`,
`session_records=8`, and `session_wire_records=8`. Covered RX-worker, GSO
xmit, plaintext outer-GSO, plaintext xmit, stale-wire, missing-session,
missing-wire, and queue-drop counters were zero. Both gates also reported the
full plaintext datapath provider active and eight warmed sessions on each
peer, with `data_path.counters.session_dial_errors=0` and
`data_path.counters.session_heartbeat_timeouts=0`.

## 2026-06-27

<a id="2026-06-27-zaozhuang-pve-e02d15e-openwrt-24107-debian-full-kmod-3600s-production-gate"></a>

### Zaozhuang PVE e02d15e OpenWrt-Debian full-kmod 3600s production gate

PVE host `120.220.44.72:8006` was used with disposable VM IDs 200+ only:
VM200 `trustix-owrt-a` ran OpenWrt 24.10.7 kernel `6.6.141`, and VM201
`trustix-deb-b` ran Debian 13 kernel `6.12.90+deb13.1-cloud-amd64` on
isolated `vmbr3`. VM100 and all 1xx guests were not modified. Both guests had
4 vCPUs and virtio network devices.

The release used TrustIX version `trustix-linux-amd64`, commit
`e02d15edf6b4`, Go `1.25.0`, build time `2026-06-27T09:02:59Z`, binary
SHA256 `915fec0c01bfee64d8440d6296bdba406bf43c602b8c428c277272560ed64d6f`,
and embedded assets SHA256
`4e0476a6fb315983cfb57f4dcb2c221e18d720aee4c90fc8c6c783f6d434681e`.
The OpenWrt `trustix_datapath.ko` SHA256 was
`005fee841ca6cb82b030bd31abac799f9e9dbd7ce7d2b5ceda340612c0c91fce`; the
Debian `trustix_datapath.ko` SHA256 was
`21304e9964d10b2284abf12fb234a140260b7929fab3508033845acf6847be5e`.
...
verifier SHA256 was
`bd01ec1a0cd9463e401e73c570e8e688d6126d5891626367895979aa4d9ec26b`.
The runner dmesg collection path was also fixed for BusyBox/OpenWrt: after
`dmesg --since` fails, it falls back to `dmesg -T` and then plain `dmesg`.

| Direction | Gate | Received | Sent | Evidence seconds | Retransmits |
| --- | ---: | ---: | ---: | ---: | ---: |
| OpenWrt to Debian | 3 Gbps | 3.548727 Gbps | 3.548818 Gbps | 3600.051435 | 314643 |
| Debian to OpenWrt | 3 Gbps | 4.909885 Gbps | 4.909995 Gbps | 3600.067996 | 48566 |

The `forward + both` run lasted 7204 seconds, from `2026-06-27T09:23:26Z` to
`2026-06-27T11:23:30Z`, with `iperf_parallel=8`. The gate passed with
`errors=[]`, `log_findings=[]`, `kernel_log_rejected_artifacts=[]`, and
`pstore_rejected_artifacts=[]`. OpenWrt boot ID
`0f82917c-294e-455b-8e77-b1df1f85a25b` and Debian boot ID
`92a6dd3e-d454-499f-bea8-04ec8a152188` were unchanged before and after the
run.

Both peers loaded `trustix_datapath`, kept `tix-lan` `tx_queue_len=1000`, and
reported `enable_features=128`, `safe_features=128`, `unsafe_features=0`,
`selftest_failures=0`, `rx_worker_inject=Y`, `tx_plaintext=Y`,
`session_records=8`, and `session_wire_records=8`. Covered RX-worker, GSO
xmit, plaintext outer-GSO, plaintext xmit, stale-wire, and queue-drop error
counters were zero. Both peers also reported
`data_path.counters.session_dial_errors=0` and
`data_path.counters.session_heartbeat_timeouts=0`.

<a id="2026-06-27-zaozhuang-pve-973a020-kmod-6-12-94-3600s-production-gates"></a>

### Zaozhuang PVE 973a020 Debian kernel fast-path 3600s production gates

PVE host `120.220.44.72:8006` was used with disposable VM IDs 200+ only:
VM202 and VM203 ran Debian 13 on isolated `vmbr3`. VM100 and all 1xx guests
were not modified. Both guests ran kernel `6.12.94+deb13-cloud-amd64`.

The release used TrustIX version `trustix-current-973a020`, commit `973a020`,
Go `1.25.0`, build time `2026-06-26T18:32:30Z`, binary SHA256
`6a96deffc0086b9cdfdf7db0eacb40b8a8b6efc6ef4a95aa37411e10b328d94a`, and
embedded assets SHA256
`4e0476a6fb315983cfb57f4dcb2c221e18d720aee4c90fc8c6c783f6d434681e`.
The Debian `trustix_datapath.ko` staged for kernel `6.12.94` had SHA256
`a7188fdaa2d0a5d98af56b57f2c4cdde3fece274741b092bfd56beb80b938cd8`.
...
| Gate family | Direction | Gate | Received | Sent | Evidence seconds | Retransmits |
| --- | --- | ---: | ---: | ---: | ---: | ---: |
| `full_kmod` | A to B | 3 Gbps | 3.423127 Gbps | 3.423167 Gbps | 3600.018335 | 312281 |
| `full_kmod` | B to A | 3 Gbps | 3.490581 Gbps | 3.490614 Gbps | 3600.003441 | 317734 |
| `tc_direct` | A to B | 3 Gbps | 4.301701 Gbps | 4.301734 Gbps | 3600.010217 | 696409 |
...
Each case ran `forward + both` with `iperf_parallel=8`; the per-case elapsed
time was 7203 or 7204 seconds. All five gates passed with `errors=[]` and
`log_findings=[]`. VM202 boot ID `86fc5a17-b38b-430b-adec-53505c3a1305` and
VM203 boot ID `79745a98-b68c-4a47-bd31-a96b42a5d9ee` were unchanged before and
after each case. Kernel log and pstore artifacts were clean on both guests.

The full plaintext datapath provider and eight warmed sessions were active for
`full_kmod`. `rx_worker_inject=Y`, `tx_plaintext=Y`,
...
<a id="2026-06-27-zaozhuang-pve-973a020-openwrt-24107-debian-full-kmod-attempt"></a>

### Zaozhuang PVE 973a020 OpenWrt-Debian full-kmod attempt

The same PVE host was used with VM204 OpenWrt 24.10.7 kernel `6.6.141` and
VM205 Debian 13 kernel `6.12.94+deb13-cloud-amd64`. The first 3600s
OpenWrt-Debian `owdeb_full_kmod` attempt used the same TrustIX build
(`973a020`) and staged OpenWrt `trustix_datapath.ko` SHA256
`005fee841ca6cb82b030bd31abac799f9e9dbd7ce7d2b5ceda340612c0c91fce` plus
Debian `trustix_datapath.ko` SHA256
`a7188fdaa2d0a5d98af56b57f2c4cdde3fece274741b092bfd56beb80b938cd8`.

The run did not produce production evidence because throughput was below the
3 Gbps OpenWrt full-kmod gate: OpenWrt to Debian measured `0.548679 Gbps`
received, and Debian to OpenWrt measured `0.883587 Gbps` received. The failure
was not a kernel panic or lockup. Boot IDs stayed stable, pstore was empty or
unavailable on OpenWrt, and covered full-kmod error counters stayed zero:
`rx_worker_xmit_ret_errors=0`, `rx_worker_gso_xmit_errors=0`,
`tx_plaintext_xmit_errors=0`, `tx_plaintext_outer_gso_errors=0`, and
`tx_plaintext_queue_drops=0`.

The low result was attributed to the disposable OpenWrt VM configuration: VM204
was initially created with `e1000` NICs and only 2 vCPUs. It was changed to
virtio NICs and 4 vCPUs for follow-up smoke testing, but that follow-up did not
complete as a production gate in this run. Therefore the current OpenWrt
production boundary remains the earlier manifest-backed OpenWrt 24.10.7 to
Debian gate at commit `395b2ba05013`.
...
<a id="2026-06-26-zaozhuang-pve-fa207ea-tc-direct-secure-kudp-3600s-production-gates"></a>

### Zaozhuang PVE fa207ea TC-direct and secure-kUDP 3600s production gates

PVE host `120.220.44.72:8006` was used with disposable VM IDs 200+ only:
VM200 `trustix-kudp-a` and VM201 `trustix-kudp-b`. VM100 and all 1xx guests
were not modified. Both guests ran Debian 13 on
`6.12.94+deb13-cloud-amd64`.

The release used TrustIX version `trustix-current-fa207ea`, commit
`fa207ea`, Go `1.25.0`, build time `2026-06-25T18:13:14Z`, binary SHA256
`e4d54d70a52cde32097229210b131cf1511da1e31fbed033877ca9d2157188c5`, and
embedded assets SHA256
`48478cb5bcca3d284964606be64b4bf0447ae6eb8a7cfd32d2408e3ff88051e7`.

Embedded Debian module hashes built for the running guest kernel:

| Module | SHA256 |
| --- | --- |
| `trustix_crypto.ko` | `1b35c7232710948a154b87c1e98f8e8e3352827204db37d8fa88cdea64379423` |
| `trustix_datapath.ko` | `b04932c552733ad35e797d5cb0b487a178fe7a4a1c59638db151c4db930642f6` |
| `trustix_datapath_helpers.ko` | `e15b8623a7ce317f2b0a980ae320d516412e0d2b0f64de49b15206ee6890a09b` |
...
| Gate family | Direction | Gate | Received | Sent | Evidence seconds | Retransmits |
| --- | --- | ---: | ---: | ---: | ---: | ---: |
| `tc_direct` | A to B | 3 Gbps | 3.765459 Gbps | 3.765510 Gbps | 3600.003746 | 478907 |
| `tc_direct` | B to A | 3 Gbps | 3.978645 Gbps | 3.978687 Gbps | 3600.003616 | 430623 |
| `secure_kudp` | A to B | 1.5 Gbps | 1.709080 Gbps | 1.709133 Gbps | 3600.013156 | 78915 |
| `secure_kudp` | B to A | 1.5 Gbps | 1.747314 Gbps | 1.747367 Gbps | 3600.013687 | 80487 |

The TC-direct `forward + both` run lasted 7204 seconds, from
`2026-06-25T23:05:45Z` to `2026-06-26T01:05:49Z`. The secure-kUDP
`forward + both` run lasted 7204 seconds, from `2026-06-26T01:06:22Z` to
`2026-06-26T03:06:26Z`. Both passed with `errors=[]` and `log_findings=[]`.
VM200 boot ID `255c8f48-850c-42ff-9928-ec99270f6996` and VM201 boot ID
`79890ca1-d112-42bd-a234-3cec1443f7ee` were unchanged before and after both
runs. Kernel log and pstore artifacts were clean on both guests, and both
`tix-lan` interfaces had `tx_queue_len=1000`.

The TC-direct gate loaded no TrustIX kernel modules. Both peers reported
`kernel_udp.provider=tc_direct`, `kernel_udp.fast_path=true`,
`kernel_udp.direct_only=true`, eight active kernel UDP flows, and zero session
dial or heartbeat failures.

The secure-kUDP gate loaded `trustix_crypto` and `trustix_datapath_helpers`,
not the full plaintext `trustix_datapath` module. Both peers reported
`kernel_udp.effective_crypto=kernel`, TC secure direct TX/RX attachment,
route-GSO kfunc use, and eight kernel crypto flow-map entries. `trustix_crypto`
reported SIMD fastpath and IRQ-FPU fastpath enabled. Direct kfunc activity was
nonzero: A reported 698526668 seal calls and 709667600 open calls; B reported
709747585 seal calls and 698447853 open calls. Helper route-GSO async xmit
packets were nonzero on both peers: 587308862 on A and 600451578 on B.

All gated secure-kUDP error counters were zero: provider unavailable errors,
flow/frame rejects, frame seal/open errors, direct kfunc errors, TX secure
direct drops/encrypt/sequence errors, RX secure direct decrypt/header errors,
and route-GSO async alloc/clone/flow/MTU/plan/prepare/segment/stream/outer-GSO,
cross-item, and xmit errors. Replay-old drops were zero on both peers. Bounded
replay-seen and total secure-direct drop ratios stayed below `0.00002`:
...
guests were not modified. Both guests ran Debian 13 on
`6.12.94+deb13-cloud-amd64`.

The release used TrustIX version `trustix-current-fa207ea`, commit
`fa207ea`, Go `1.25.0`, build time `2026-06-25T18:13:14Z`, binary
SHA256 `e4d54d70a52cde32097229210b131cf1511da1e31fbed033877ca9d2157188c5`,
and embedded assets SHA256
`48478cb5bcca3d284964606be64b4bf0447ae6eb8a7cfd32d2408e3ff88051e7`.
The release tarball SHA256 was
`0d9f8989d3e7a8bf4623746bef14b17183a212e54280a206a0ae21c50807abe1`.
Later local evidence commits only update test diagnostics and evidence
metadata; this gate exercised the `fa207ea` runtime binary.
...
| Direction | Gate | Received | Sent | Evidence seconds | Retransmits |
| --- | ---: | ---: | ---: | ---: | ---: |
| A to B | 1.5 Gbps | 1.703143 Gbps | 1.703198 Gbps | 3600.008549 | 66412 |
| B to A | 1.5 Gbps | 1.662160 Gbps | 1.662217 Gbps | 3600.014982 | 62811 |

The sequential `forward + both` run lasted 7203 seconds, from
`2026-06-25T18:24:42Z` to `2026-06-25T20:24:45Z`, and passed with
`errors=[]` and `log_findings=[]`. VM200 boot ID
`e9bf9f8f-58bd-4505-811f-0dc236b7342e` and VM201 boot ID
`9072dfbd-426d-4c9d-a420-4acb578a7880` were unchanged before and after the
...
on both peers: A reported 697313566 seal calls and 684535661 open calls; B
reported 684598283 seal calls and 697247517 open calls.

All gated error counters were zero: direct kfunc errors, kernel crypto provider
unavailable errors, flow/frame rejects, frame seal/open errors, replay drops,
TX packet seal errors, XDP crypto open/decrypt/header/payload/replay/store
errors, and route-GSO async flow/plan/MTU/alloc/clone/segment/prepare/xmit,
stream, outer-GSO, cross-item, and tail-stitch errors. Route-GSO async xmit
packet counters were nonzero on both peers: 42690272 on A and 42551677 on B.

<a id="2026-06-26-zaozhuang-pve-fa207ea-route-gso-3600s-production-gate"></a>

### Zaozhuang PVE fa207ea route-GSO 3600s production gate

PVE host `120.220.44.72:8006` was used with disposable VM IDs 200+ only:
VM200 `trustix-rg-a` and VM201 `trustix-rg-b`. VM100 and all 1xx guests were
not modified. Both guests ran Debian 13 on `6.12.94+deb13-cloud-amd64`.
...
embedded assets SHA256
`48478cb5bcca3d284964606be64b4bf0447ae6eb8a7cfd32d2408e3ff88051e7`.
This is the plaintext route-GSO production gate for the `fa207ea` runtime,
including the direct route-GSO TCP segment-header fix.
...
| Direction | Gate | Received | Sent | Evidence seconds | Retransmits |
| --- | ---: | ---: | ---: | ---: | ---: |
| A to B | 2.5 Gbps | 2.975477 Gbps | 2.975519 Gbps | 3600.014695 | 724700 |
| B to A | 2.5 Gbps | 3.004100 Gbps | 3.004146 Gbps | 3600.015931 | 738958 |

The sequential `forward + both` run lasted 7204 seconds, from
`2026-06-25T20:41:55Z` to `2026-06-25T22:41:59Z`, and passed with
`errors=[]` and `log_findings=[]`. VM200 boot ID
`b1d92977-3dca-4fa3-b6fc-28e5a5af560d` and VM201 boot ID
`829f66cf-ed57-461f-89d3-4b52dbe3f579` were unchanged before and after the
run. Kernel log and pstore artifacts were clean on both guests, and both
`tix-lan` interfaces had `tx_queue_len=1000`.
...
Route-GSO module counters showed active outer-GSO and no covered route-GSO
errors:

| Node | Outer-GSO frames | Xmit packets | Covered helper errors |
| --- | ---: | ---: | ---: |
| A | 1008002417 | 43311681 | 0 |
| B | 1017681020 | 43927358 | 0 |

## 2026-06-25

<a id="2026-06-25-zaozhuang-pve-openwrt-24107-current-head-full-kmod-3600s-production-gate"></a>

### Zaozhuang PVE OpenWrt 24.10.7 current-head full-kmod 3600s production gate

PVE host `120.220.44.72:8006` was used with disposable VM IDs 200+ only:
VM200 ran OpenWrt 24.10.7 and VM201 ran Debian 13 on isolated `vmbr3`. VM100
and all 1xx guests were not modified. OpenWrt ran kernel `6.6.141`; Debian ran
`6.12.94+deb13-cloud-amd64`.

The release used TrustIX version `trustix-linux-amd64`, commit
`395b2ba05013`, Go `1.25.0`, build time `2026-06-25T02:09:58Z`, binary SHA256
`552f92fd9884502e408db0c65b865d14bb6d63665b9c2a6533a0f643d3c31104`, and
embedded assets SHA256
`18eb4b0fbb81b7dfe6a9639e2997cae6cd728c5a9d2db3ba367412487cb6e622`.

OpenWrt modules were built from the matching 24.10.7 SDK:

| Module | SHA256 |
| --- | --- |
| `trustix_crypto.ko` | `896a03bbf2959f2f53f9daeb6ede04607399d061159447bbf20eda2cbb39fac7` |
| `trustix_datapath.ko` | `005fee841ca6cb82b030bd31abac799f9e9dbd7ce7d2b5ceda340612c0c91fce` |
| `trustix_datapath_helpers.ko` | `b4c951fb46d1dcc8dbe9051a50dd31898101bd1116ef054f7b75dda3bb294bf0` |

The Debian full-kmod run used `trustix_datapath.ko` SHA256
`78f2999765c35fbb7298e49bd15788a7328cfb68f1b190a773be3cd48b3eb0f0`, built
for the running Debian kernel.
...
| Direction | Gate | Received | Sent | Evidence seconds | Retransmits |
| --- | ---: | ---: | ---: | ---: | ---: |
| OpenWrt to Debian | 3 Gbps | 3.493808 Gbps | 3.493850 Gbps | 3599.998937 | 311205 |
| Debian to OpenWrt | 3 Gbps | 5.108514 Gbps | 5.108586 Gbps | 3600.038259 | 71637 |

The sequential `forward + both` run lasted 7204 seconds, from
`2026-06-25T02:26:21Z` to `2026-06-25T04:26:25Z`, and passed with
`errors=[]` and `log_findings=[]`. OpenWrt boot ID
`f48bb4e2-8896-4d16-b139-12ae56ceade7` and Debian boot ID
`e23a85b2-f416-45e5-b4ff-8da7a333d298` were unchanged before and after the
