...
`scripts/linux-full-datapath-module-smoke.sh` 会在目标内核上构建并加载 `kernel/trustix_datapath`，验证 `/dev/trustix_datapath`、sysfs `selftests=4095` / `selftest_failures=0`、route/session/session-wire/flow state ABI、packet classify、TIXT encap/decap、基于 session-wire 的受控外层 IPv4 UDP/TIX-TCP 构包/解析 ioctl、veth ingress 下的 RX_STAGE staging ring peek/pop、RX_WORKER 注入和 TX_PLAINTEXT plaintext LAN 封装。请求 `full_datapath` 时脚本默认补 `enable_features=7296 rx_worker_inject=1 tx_plaintext=1`，期望 `features=7296` / `safe_features=7296` / `unsafe_features=0`。bit 10 只在 TIX-TCP 双端协商成功后使用内层 TCP `CHECKSUM_PARTIAL`；bit 11 在双方同时支持且本机 GSO/GRO 能力可用时保留 inner TCP GSO，并在接收端重建 GSO skb；bit 12 将内层流稳定分散到 16 个协商式外层源端口和独立序号域。旧节点、协商失败或运行时能力撤销时会自动回退到原端口和分段路径，`TRUSTIX_TIX_TCP_INNER_GSO=0`、`TRUSTIX_TIX_TCP_PORT_SHARDING=0` 可分别关闭 bit 11、bit 12。daemon 在模块已加载且配置了 underlay interface 时可用 `RX_PREVIEW|RX_STAGE` 继续走 hybrid staging，也可在 `TRUSTIX_KERNEL_DATAPATH_FULL_PLAINTEXT=1` 时同时 attach underlay RX worker hook 和 LAN TX plaintext hook，让 plaintext 数据面两方向都在 `.ko` 内完成。
