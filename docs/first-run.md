...
- `TRUSTIX_SECURE_TLS_HANDSHAKE_ONLY=1` 可显式启用 secure TCP 的协商式 TLS data-plane handoff。双方仍先完成 TLS 握手、证书校验和 TrustIX hello，并继续使用 TrustIX AEAD、sequence anti-replay 和 TLS exporter/X25519 派生的数据密钥；切换屏障完成后，数据记录直接承载在原始 TCP stream 上，避免重复的 TLS record AEAD。该能力只会在双方都启用、底层为可脱离的 TLS stream 且 TrustIX 双向加密完整开启时协商；旧版本、单端启用、plaintext/单向加密或非 TCP stream 会保留完整 TLS。当前默认关闭，`TRUSTIX_SECURE_TLS_HANDSHAKE_ONLY=0` 是显式回退；已协商 session 会在 transport stats 的 `extra.tls_handshake_only=1` 中标记。`link_tls` 仍表示该 session 已通过 TLS 建立并保留 TLS identity/exporter 状态，不表示切换后的每个 data record 都是 TLS record。
