...
- 需要低层性能证据时，可用 `TRUSTIX_CPU_PROFILE_DIR=<dir>` 持续采集 CPU profile；用 `TRUSTIX_RUNTIME_PROFILE_DIR=<dir>` 在 daemon 正常退出时写出 heap profile。只有同时设置正整数 `TRUSTIX_MUTEX_PROFILE_FRACTION` 或 `TRUSTIX_BLOCK_PROFILE_RATE` 时才启用 mutex/block 采样并在退出时写盘。profile 文件以 `0600` 创建；采样会增加开销，不应作为常驻默认配置。
- `transport_policy.tls_data_plane` 控制 secure TCP 的 TLS data-plane handoff，生产默认值是 `auto`。双方仍先完成 TLS 握手、证书校验和 TrustIX hello，并继续使用 TrustIX AEAD、sequence anti-replay 和 TLS exporter/X25519 派生的数据密钥；切换屏障完成后，数据记录直接承载在原始 TCP stream 上，避免重复的 TLS record AEAD。该能力只会在双方都支持、底层为可脱离的 TLS stream 且 TrustIX 双向加密完整开启时协商；旧版本、单端不支持、plaintext/单向加密或非 TCP stream 会保留完整 TLS。`full_tls` 显式关闭 handoff。`TRUSTIX_SECURE_TLS_HANDSHAKE_ONLY=0` 可在事故处置时强制完整 TLS，`1` 可强制允许协商；显式环境变量优先于配置。已协商 session 会在 `transport_tls.tls_data_plane_handoff_sessions` 和 transport stats 的 `extra.tls_handshake_only=1` 中标记。`link_tls` 仍表示该 session 已通过 TLS 建立并保留 TLS identity/exporter 状态，不表示切换后的每个 data record 都是 TLS record。
- Linux 原生 TCP stream 默认会在一次阻塞读取后，以 `TIOCINQ` 和 nonblocking `MSG_PEEK` 检查内核接收队列，并继续读取已经完整到达的 TrustIX record。该路径不等待新数据、不增加 timer、不改变 wire format；探测失败、非 Linux、非 `syscall.Conn` 或尚未完成 TLS data-plane handoff 的连接会直接保留旧读取路径。`TRUSTIX_STREAM_KERNEL_QUEUE_DRAIN=0`（也接受 `false`/`off`/`disabled`）可显式回退；active session 会报告 `extra.stream_kernel_queue_drain=1`、续读次数和字节数。
...
- 双节点单活网关使用 keepalived/VRRP 漂移 LAN gateway 与所有 endpoint VIP，并由 `trustix-ha.sh` 通过 fencing、离线预检和 `/readyz` 控制同一逻辑 IX 的唯一 daemon；拓扑约束、脑裂边界和演练矩阵见 [ha-active-standby.md](ha-active-standby.md)。
...
`scripts/linux-full-datapath-module-smoke.sh` 会在目标内核上构建并加载 `kernel/trustix_datapath`，验证 `/dev/trustix_datapath`、sysfs `selftests=4095` / `selftest_failures=0`、route/session/session-wire/flow state ABI、packet classify、TIXT encap/decap、基于 session-wire 的受控外层 IPv4 UDP/TIX-TCP 构包/解析 ioctl、veth ingress 下的 RX_STAGE staging ring peek/pop、RX_WORKER 注入和 TX_PLAINTEXT plaintext LAN 封装。该单机 smoke 的独立基线仍是 `enable_features=5248 rx_worker_inject=1 tx_plaintext=1`；设置 `TRUSTIX_FULL_DATAPATH_ENABLE_FEATURES=7296` 可额外验证 bit 11。实际 daemon 选中 plaintext full-kmod performance profile 时默认加载 `enable_features=7296 inner_gso_auto_recover=0`：bit 10/11 只在 TIX-TCP 双端 capability 和 kernel session readiness 都提交后保留内层 TCP `CHECKSUM_PARTIAL` 与 inner GSO，bit 12 将内层流稳定分散到 16 个协商式外层源端口和独立序号域。运行时 frame fault、重组 timeout 比例或持续无进展会触发能力撤销；双方 ACK 并排空旧帧后，原会话锁存回退到 outer-GSO，不自动恢复。`TRUSTIX_TIX_TCP_INNER_GSO=0` 可显式选择 `5248` outer-GSO profile，旧节点或协商失败也会使用该兼容路径；`TRUSTIX_TIX_TCP_PORT_SHARDING=0` 会同时关闭 bit 11/12。daemon 在模块已加载且配置了 underlay interface 时可用 `RX_PREVIEW|RX_STAGE` 继续走 hybrid staging，也可在 `TRUSTIX_KERNEL_DATAPATH_FULL_PLAINTEXT=1` 时同时 attach underlay RX worker hook 和 LAN TX plaintext hook，让 plaintext 数据面两方向都在 `.ko` 内完成。
...
transport_policy:
  ...
  encryption: secure
  # auto | full_tls
  # auto 在 TLS 身份认证后协商切换到 TrustIX AEAD 数据面。
  tls_data_plane: auto
