...
`scripts/linux-datapath-module-smoke.sh` 会在目标内核上构建并加载 `kernel/trustix_datapath_helpers`，验证 `/dev/trustix_datapath_helpers`、sysfs `selftests=3` / `selftest_failures=0`、TIXT selftest OK flag、`gso_skb` safe feature，以及 Go ioctl query/selftest ABI。脚本会断言 route-TCP XMIT worker 保持关闭；route-TCP GSO async、outer-GSO batch 和 TIXT RX stream/coalesce 第一版由模块 init 与 daemon 参数过滤共同 hard-disable。设置 `TRUSTIX_DATAPATH_ENABLE_FEATURES=0` 可验证 feature gate 禁用路径。
