...
`scripts/linux-full-datapath-module-smoke.sh` 会在目标内核上构建并加载 `kernel/trustix_datapath`，验证 `/dev/trustix_datapath`、sysfs `selftests=2047` / `selftest_failures=0`、route/session/session-wire/flow state ABI、packet classify、TIXT encap/decap、基于 session-wire 的受控外层 IPv4 UDP/TIX-TCP 构包/解析 ioctl、veth ingress 下的 RX_STAGE staging ring peek/pop、RX_WORKER 注入和 TX_PLAINTEXT plaintext LAN 封装。请求 `full_datapath` 时脚本会自动补 `enable_features=1152 rx_worker_inject=1 tx_plaintext=1`，期望 `features=1152` / `safe_features=1152` / `unsafe_features=0`。其中 bit 10 只在 TIX-TCP 双端协商成功后使用内层 TCP `CHECKSUM_PARTIAL`，新旧版本混跑会自动保留完整 checksum；接收目标不支持 checksum offload 时由模块软件完成。daemon 在模块已加载且配置了 underlay interface 时可用 `RX_PREVIEW|RX_STAGE` 继续走 hybrid staging，也可在 `TRUSTIX_KERNEL_DATAPATH_FULL_PLAINTEXT=1` 时同时 attach underlay RX worker hook 和 LAN TX plaintext hook，让 plaintext 数据面两方向都在 `.ko` 内完成。
