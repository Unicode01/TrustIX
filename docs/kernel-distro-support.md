...
| Datapath helpers, full | Matching KDIR plus module BTF/kfunc usability | Provides safe skb/GSO and route-TCP helper capability. Route-TCP GSO/XMIT families are enabled only by selected policies after runtime parameter checks and cross-host production validation; otherwise they fail closed. |
| Full plaintext datapath module | Matching KDIR; selected plaintext full-kmod policy plus `enable_features=7296 inner_gso_auto_recover=0 rx_worker_inject=1 tx_plaintext=1`; passing module selftests and `tix_tcp_inner_gso` cross-host production gate | Selected plaintext performance path for the validated Debian 6.12 and OpenWrt 5.15/6.6 x86_64 targets. Bits 10/11 preserve negotiated inner TCP `CHECKSUM_PARTIAL` and inner GSO; bit 12 distributes inner flows over 16 negotiated outer source ports and sequence domains for RSS/GRO locality. Both peers must commit kernel session readiness before inner-GSO is used. A runtime fault withdraws the capability with ACK plus bounded drain and locks the existing session onto resilient outer-GSO framing; automatic recovery is disabled. `TRUSTIX_TIX_TCP_INNER_GSO=0` selects the previous `5248` outer-GSO profile, and `TRUSTIX_TIX_TCP_PORT_SHARDING=0` disables bits 11/12. Daemon defaults still keep RX worker and TX plaintext disabled unless the selected policy/runtime config requires them. Modules remain target-kernel artifacts. |
| Secure TIX-TCP full datapath | Matching `trustix_crypto` and `trustix_datapath` artifacts; bits 13/14; AES-NI/runtime readiness; passing selftests and secure cross-host production gate | Implemented and exercised on Debian 6.1/6.12 public hosts. Secure RX uses a preallocated 65,535-byte per-CPU scratch buffer for nonlinear/GRO packets, so it does not linearize the source skb in atomic context. This remains an opt-in candidate until a strong-identity, 4-vCPU, 3600-second gate passes; all aggregate, replay, writable, parse, crypto, checksum, copy, layout, and delivery errors must remain zero. |
...
`6.12.96+deb13-cloud-amd64`, OpenWrt 23.05.5 x86_64 `5.15.167`, OpenWrt
23.05.6 x86_64 `5.15.189`, OpenWrt 24.10.2 x86_64 `6.6.93`, OpenWrt
24.10.7 x86_64 `6.6.141`,
...
| `trustix_datapath_helpers` | pass | Full helper loaded, selftests passed, `gso_skb` active. Baseline helper smoke kept route-TCP XMIT disabled; production route-GSO uses separate cross-host gates. |
...
sessions active. Later commits `5ee74be`, `0fb27b8`, `559145e`, and
...
TC secure direct and route-GSO kfunc stats were active on both peers, crypto
module seal/open counters were nonzero, helper route-GSO xmit counters were
nonzero, boot IDs stayed stable, and pstore plus kernel log scans were clean.
...
A 2026-07-08 current-head OpenWrt 24.10.7-to-Debian full-kmod recheck paired
OpenWrt kernel `6.6.141` with Debian 13
`6.12.94+deb13-cloud-amd64` and passed the same 3600s-per-direction production
...
zero. This was superseded for TIX-TCP by the 2026-07-30 gate below; it remains
the current UDP full-kmod production evidence for Debian
`6.12.94+deb13-cloud-amd64`.

A 2026-07-30 TIX-TCP full-kmod refresh paired OpenWrt 24.10.7 kernel
`6.6.141` with Debian 13 `6.12.96+deb13-cloud-amd64`, while a concurrent
Debian-to-Debian pair used the same Debian kernel. Commit
`321cd1d549a493cd415451e48f6ddcd9ae089f50` replaced repeated outer-GSO
backing allocation with reusable per-CPU page pools. The Debian pair passed at
11.474420 and 10.498864 Gbps; the OpenWrt-Debian pair passed at 5.869672 and
5.909833 Gbps. Every direction contained 3600 intervals, boot IDs stayed
stable, pstore and kernel-log checks were clean, and page-pool errors were zero.
Linux 6.12 uses `page_pool_create_percpu()` while 5.15/6.6 use independent
per-CPU `page_pool_create()` instances, with `alloc_skb()` retained as a safe
