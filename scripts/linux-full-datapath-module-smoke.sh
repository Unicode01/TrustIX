...
enable_features="${TRUSTIX_FULL_DATAPATH_ENABLE_FEATURES:-7296}"
...
has_inner_tcp_checksum_partial_bit() {
  local value="$1"
  [[ "$value" =~ ^[0-9]+$ ]] || return 1
  [[ "$(( (value / 1024) % 2 ))" -eq 1 ]]
}
...
verify_sysfs() {
  ...
  if truthy "$expect_active"; then
    ...
    if has_inner_tcp_checksum_partial_bit "$enable_features"; then
      if ! has_inner_tcp_checksum_partial_bit "$features" || ! has_inner_tcp_checksum_partial_bit "$safe"; then
        die "inner TCP checksum-partial feature did not become safe and active: features=${features} safe=${safe} unsafe=${unsafe}"
      fi
    ...
    if has_tix_tcp_port_sharding_bit "$enable_features"; then
      if ! has_tix_tcp_port_sharding_bit "$features" || ! has_tix_tcp_port_sharding_bit "$safe"; then
        die "TIX-TCP port-sharding feature did not become safe and active: features=${features} safe=${safe} unsafe=${unsafe}"
      fi
