...
trustix_wizard_temp_repo=0

trustix_wizard_cleanup_temp_repo() {
  local status="$1"
  if [[ "$status" != "0" && "${trustix_wizard_temp_repo:-0}" == "1" && "${repo_root:-}" == /tmp/trustix-wizard-src.* ]]; then
...
trustix_wizard_extract_archive() {
  local archive_path="$1"
  local dest="$2"
  local stage archive_root candidate
  stage="$(mktemp -d /tmp/trustix-wizard-archive.XXXXXX)"
...
trustix_wizard_mirrors_enabled() {
...
trustix_wizard_github_url_candidates() {
  local url="$1"
  local mirror mirrors
  printf '%s\n' "$url"
  trustix_wizard_mirrors_enabled || return 0
  case "$url" in
...
if ! repo_root="$(wizard_repo_root)"; then
  ...
  if [[ ! -f "${repo_root}/go.mod" ]]; then
    ...
    if command -v git >/dev/null 2>&1; then
      if git clone --depth 1 --branch "$repo_ref" "$repo_url" "$repo_root" >&2; then
        ...
        trustix_wizard_log "git clone failed, trying GitHub archive mirrors"
        rm -rf "$repo_root"
        mkdir -p "$repo_root"
      fi
    fi
    if [[ "$cloned" != "1" ]] && { command -v curl >/dev/null 2>&1 || command -v wget >/dev/null 2>&1; } && command -v tar >/dev/null 2>&1 && [[ "$repo_url" == https://github.com/* ]]; then
      ...
      mapfile -t archive_urls < <(trustix_wizard_github_url_candidates "$archive_url")
      trustix_wizard_download_file "$archive_path" "${archive_urls[@]}"
      trustix_wizard_extract_archive "$archive_path" "$repo_root"
      rm -f "$archive_path"
      cloned=1
    fi
    if [[ "$cloned" != "1" ]]; then
  ...
  child_status=0
  bash "${repo_root}/scripts/trustix-wizard.sh" "$@" || child_status=$?
  trustix_wizard_cleanup_temp_repo "$child_status"
  trap - EXIT
  exit "$child_status"
fi
